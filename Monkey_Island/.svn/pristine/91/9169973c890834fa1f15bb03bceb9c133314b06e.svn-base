...
/**
 * Classe du client TCP à utiliser avec JMeter pour tester un serveur.
 * 
 * <p>Peut être utilisé pour tester un serveur Java 
...
public class TCPClientImpl implements TCPClient
{
	...
	public void write(OutputStream os, String s) 
	{
		try {
			if (s.length() != 0) {
				final BufferedWriter buffer = new BufferedWriter(new OutputStreamWriter(os, ENCODAGE));
	...
	public String read(InputStream is)
	{
		...
		try {
			buffer = new BufferedReader(new InputStreamReader(is, ENCODAGE));
			...
			System.out.print("[info] TCPClient : Interruption d'expiration sur le flux ");
			System.out.println("(" + ex.getMessage() + ")");
		}
		catch (UnsupportedEncodingException ex) {
			System.err.println("Problème de lecture sur le flux, encodage " + ENCODAGE + " non supporté.");
			System.err.println(ex.getMessage());
		}
		catch (IOException ex) {
